    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.beliefs:
            ...
            for q in posDist:
                new[q] += self.beliefs[p] * posDist[q]
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        for p in self.particles:
            beliefs[p] += emissionModel[util.manhattanDistance(pacmanPosition, p)]
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        weight with each position) is incorrect and may produce errors.
        """
        pairs = [(p1, p2) for p1 in self.legalPositions for p2 in self.legalPositions]
        self.particles = [pairs[i % len(pairs)] for i in range(self.numParticles)]
        ...
        for p in self.particles:
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] is None:
                    ...
                    prob *= emissionModels[i][util.manhattanDistance(pacmanPosition, p[i])]
            beliefs[tuple(temp)] += prob
