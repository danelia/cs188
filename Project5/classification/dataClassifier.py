...
def enhancedFeatureExtractorDigit(datum):
    ...
    for i in range(len(pixels)):
        for j in range(len(pixels[i]) - 1):
            if pixels[i][j + 1] != 0:
                nonzero += 1
                if not firstLeft or j < firstLeft - 1:
                    firstLeft = j + 1
                if j <= (len(pixels) + 1) / 2 - 1:
                    aboveCenter += 1
    ...
    for j in range(len(pixels[0])):
        ...
        for i in range(len(col) - 1):
            if col[j] != 0:
                nonzero += 1
                if not firstTop or i < firstTop - 1:
                    firstTop = i + 1
                if i <= (len(pixels[0]) + 1) / 2 - 1:
                    pastRight += 1
            if col[i + 1] != col[i]:
                breaks += 1

    ratio = float(width) / (len(pixels) - (firstTop * 2))
    above = float(aboveCenter) / nonzero
    right = float(pastRight) / nonzero

    for n in range(5):
        features[n] = breaks > 175 and 1.0 or 0.0
        features[-n] = nonzero > 300 and 1.0 or 0.0
        features[-(n + 1) * 10] = above > 0.35 and 1.0 or 0.0
        features[1000 + n] = right < 0.27 and 1.0 or 0.0
        features[(n + 1) * 10] = ratio < 0.69
        features[(n + 6) * 10] = ratio < 0.69
...
def enhancedPacmanFeatures(state, action):
    ...
    ghosts = sorted([util.manhattanDistance(position, i) for i in successor.getGhostPositions()])
    features["ghosts"] = ghosts[0] * 1.0
    for i in xrange(min(len(ghosts), 1)):
        features[("ghost", i)] = 5 / (0.1 + ghosts[i])

    capsules = sorted([util.manhattanDistance(position, i) for i in successor.getCapsules()])
    for i in xrange(min(len(capsules), 1)):
        features[("capsules", i)] = 15 / (1 + capsules[i])

    food = [(x, y) for x in range(len(list(state.getFood()))) for y in range(len(state.getFood()[x])) if state.getFood()[x][y]]
    food = sorted([util.manhattanDistance(position, i) for i in food])
    for i, weight in zip(xrange(min(len(food), 5)), [1.3, 0.8] + [0.9] * 3):
        features[("food", i)] = weight * food[i]

    features["capsuleCount"] = len(successor.getCapsules()) * 10
